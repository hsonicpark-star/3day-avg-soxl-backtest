...
def _render_account_tab(tk: str, tk_cfg: dict, key_sfx: str):
    ...
    with st.expander("💰 자본 조정 (증액 / 감액)"):
        ...
        # 직전 적용 성공 메시지 표시 + 입력칸 초기화 (중복 가드 오탐 혼란 방지)
        if f"adj_done_{key_sfx}" in st.session_state:
            st.success(st.session_state.pop(f"adj_done_{key_sfx}"))
            for _rk in (f"capital_adj_input_{key_sfx}", f"adj_memo_{key_sfx}"):
        ...
        _dup_exists_avg = any(
            str(_e.get("날짜", "")) == _adj_date.strftime("%Y-%m-%d")
            and abs(float(_e.get("조정금액", 0) or 0) - float(_adj_amount)) < 0.01
            for _e in _adj_history) if _adj_amount != 0 else False
        _dup_ok_avg = True
        if _dup_exists_avg:
            _dup_ok_avg = _adj_c1.checkbox(
                "⚠️ 같은 날짜·금액 이력이 이미 있습니다 — 실제로 한 번 더 "
                "입출금한 경우에만 체크 후 적용하세요",
                value=False, key=f"avg_dup_ok_{key_sfx}")
        if _adj_c2.button("💰 적용", use_container_width=True,
            ...
            if _cur_total + _adj_amount <= 0:
            ...
            elif _dup_exists_avg and not _dup_ok_avg:
                ...
                if _baked:
                    ...
                    st.session_state[f"adj_done_{key_sfx}"] = (
                        f"✅ {_adj_date} 자본 조정 완료. 현재 자본금: **${_final_cap:,.0f}**")
                st.rerun()
        ...
        if _adj_history:
            ...
            if "원장반영" not in _df_adj_edit.columns:
                _df_adj_edit["원장반영"] = False
            _df_adj_edit["원장반영"] = (_df_adj_edit["원장반영"]
                                         .fillna(False).astype(bool))
            _edited = st.data_editor(
                _df_adj_edit[["날짜", "조정금액", "메모", "원장반영"]],
                column_config={
            ...
            if st.button("💾 변경사항 저장", key=f"save_adj_edit_{key_sfx}",
                ...
                if _preview_cap <= 0:
                    ...
                    _bake_delta = 0.0
                    _valid_kept = set()
                    for _ix, _r in _edited.iterrows():
                        if pd.isna(_r.get("날짜")) or pd.isna(_r.get("조정금액")):
                            continue
                        if isinstance(_ix, int) and _ix < len(_adj_history):
                            _valid_kept.add(_ix)
                            _old_e = _adj_history[_ix]
                            if _old_e.get("원장반영"):
                                _bake_delta += (float(_r["조정금액"])
                                                 - float(_old_e.get("조정금액", 0) or 0))
                    for _oix, _old_e in enumerate(_adj_history):
                        if _oix not in _valid_kept and _old_e.get("원장반영"):
                            _bake_delta -= float(_old_e.get("조정금액", 0) or 0)
                    _unbake_msg = ""
                    if abs(_bake_delta) > 0.005:
                        _bk_ok2, _ = _bake_adj_into_avg_ledger(tk, _bake_delta)
                        if not _bk_ok2:
    ...
    with st.expander("🔧 현재 보유 상태 보정 (실제 계좌와 맞추기)"):
        st.caption("실제 증권사 계좌의 **현재 보유주수 · 현금**을 입력하면 매매기록 앵커를 그 값으로 "
                   "바로잡습니다. 잘못된 '예정' 행은 제거되고, 다음 주문부터 실제 보유·현금 기준으로 "
        ...
        if not _fix_led.empty and "보유주수" in _fix_led.columns:
            _conf = _fix_led[
                ...
                & _fix_led["종가(x)"].apply(lambda v: str(v).strip() not in ("", "-", "None", "nan"))]
            if not _conf.empty:
                _lastc = _conf.iloc[-1]
        ...
        if st.button("✅ 현재 상태로 보정", type="secondary", key=f"avg_do_fix_{key_sfx}"):
            ...
            if _rows_fix:
                ...
                _lf = _rows_fix[-1]
                _cx = pd.to_numeric(_lf.get("종가(x)"), errors="coerce")
                _cx = float(_cx) if pd.notna(_cx) else 0.0
                _lf["보유주수"] = int(_in_h)
                _lf["현금($)"] = round(float(_in_c), 2)
                _lf["총자산($)"] = round(float(_in_c) + int(_in_h) * _cx, 2)
            else:
                _anchor_d = (datetime.today().date() - timedelta(days=1)).strftime("%Y-%m-%d")
                _row0 = {c: "" for c in AVG_HIST_COLS}
                _row0.update({"날짜": _anchor_d, "매매": "-", "거래주수": 0, "거래금액($)": 0,
                              "보유주수": int(_in_h), "보유기간": "-",
    ...
    if st.button(_btn_label, type="primary", key=f"run_os_{key_sfx}"):
        ...
        if _led_st == "no_url":
            ...
            _led_reason = st.session_state.get(f"led_err_{tk}", "")
