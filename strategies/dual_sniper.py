...
_DS_PRESETS = [
    ...
    {"label": "🛡️ 안정형 (헬기셋)", "ag_div": 6, "ag_buy": 0.5, "ag_sell_alpha": 0.4,
     ...
     "help": ("헬기셋 — 공격 매수 0.5% + 분할6 (전차셋보다 더 분산)\n"
              "공격: 분할6 · 매수0.5% · 매도α0.4 · 보유α2.0\n"
              "방어: 로케트셋과 동일\n"
              "원전략 실제모드 기준 CAGR 75.2% / MDD -20.6% / Calmar 3.65 (MDD 최저)")},
]
...
- **1티어 보류(공통규칙)**: 전일종가 > 전일 MA(5)면 **슬롯1은 매도 보류**(MOC 제외) → 상승 초입에 첫 슬롯을 끌고 감. (2026-06-30 스펙: 공격·방어 공통)

**보유기간** — 8거래일 고정, 만기 MOC 청산.
""")

    with st.expander("💰 비중·자금 배분 로직 (티어 할당)", expanded=True):
        st.markdown("""
**컨셉**: 작게 시작 → 약할 때 키워 매집 → **현금 한도 내에서만** 운용 (레버리지 금지).

#### 📊 티어별 매수비중 (점증 램프)
각 슬롯(티어)에 들어갈 자본 비중을 **오름차순**으로 둡니다. 예: 방어 5분할 기본값
`6 → 13 → 20 → 27 → 34%` (합 100).
- **첫 티어(T1)는 6%로 아주 작게** 진입 → 사자마자 반등해도 손실 미미
- 가격이 더 빠질수록 **큰 비중**의 다음 티어가 들어옴 → **역피라미딩(물타기)** 으로 평단가 개선

#### 🧮 할당 공식 — "남은 현금"을 "남은 비중"에 비례 배분
> 📄 **원전략 공식 설명서 §4 그대로** (역설계 아님, 스펙 확정):
```
매수할당금 = 시작현금 × ( 다음티어 비중 ÷ 남은티어 비중합 )
```
- **기준은 총자산·시작자본이 아니라 "시작현금"(장 시작 시점 현금)** 입니다.
- 남은 비중합으로 정규화 → 5티어를 다 채우면 **각 티어가 정확히 시작자본의 6/13/20/27/34%**, 합계 100% 투자.
- 즉 이상적(풀현금·가격불변) 상황에선 "자본 대비 고정 비중"과 **수학적으로 동일**. 다만 매번 실제 현금으로 재계산해 **정수주·체결가 오차를 자동 보정**합니다.

#### 💵 단일 현금풀 — 공격·방어가 한 통장 공유
- 공격·방어는 **하나의 현금 잔고**를 같이 씁니다 (모드별 분리 자본 ❌).
- 모드가 전환돼도 **기존 보유분은 청산하지 않고 carry** → 각자의 매도목표에서 익절.
- 따라서 공격분이 현금을 묶고 있으면, 새 방어 티어는 **남은 현금 기준**으로 작아집니다.
  → 두 모드가 동시에 풀사이즈로 쌓여 노출이 폭발하는 것을 **구조적으로 차단** (안전장치).

> **예) 시드 $50,000, 공격 2슬롯에 $14,000 묶임 → 현금 $36,000**
> 방어 T1 = `36,000 × 6% = $2,160` (시드 6% $3,000 아님 · 총자산 6%도 아님)

#### ✅ 원전략 교차검증 (2016~2026, 198건)
| 할당 기준 가설 | 실제 시트와 평균오차 |
|---|---|
| **현재현금 × 비중** (본 엔진) | **0.73%** (정수주 잔돈 수준) |
| 총자산 × 비중 | 94.3% (불일치) |

원전략도 **70회 중 49회**를 티어 보유 중에 모드 전환했고, 그때마다 새 모드 할당이 **현금 × 비중**과 일치했습니다 — 단일 현금풀·현금기준 배분이 원전략의 실제 방식임을 확인.
