...
# ══════════════════════════════════════════════════════════════
# Sigma매매 계좌 설정 CRUD  (키 접두사 "sigma_")
# ══════════════════════════════════════════════════════════════
_SIGMA_PFX = "sigma_"

def _get_sigma_ticker_settings() -> dict:
    """등록된 Sigma매매 계좌 설정 반환 {ticker: {sigma_period, amount_per_trade, …}}"""
    if _IS_CLOUD and st.session_state.get("logged_in"):
        raw = st.session_state.get("user_settings", {}).get("sigma_ticker_settings", "") or ""
        try:    return json.loads(raw) if raw else {}
        except: return {}
    full_cfg = _load_full_config()
    return {k[len(_SIGMA_PFX):]: v
            for k, v in full_cfg.items()
            if k.startswith(_SIGMA_PFX) and isinstance(v, dict)}

def _save_sigma_ticker_setting(tk: str, data: dict) -> str:
    save_config(data, f"{_SIGMA_PFX}{tk}")
    if _IS_CLOUD and st.session_state.get("logged_in"):
        try:
            raw = st.session_state.get("user_settings", {}).get("sigma_ticker_settings", "") or ""
            ts  = _parse_ticker_settings_json(raw)
            ...
            if "user_settings" not in st.session_state:
                st.session_state.user_settings = {}
            st.session_state.user_settings["sigma_ticker_settings"] = ts_json
            _save_user_settings_to_sheet(st.session_state.username, {"sigma_ticker_settings": ts_json})
            return ""
        except Exception as e:
            return f"저장 중 오류: {e}"
    return ""
...
def _delete_sigma_ticker_setting(tk: str) -> str:
    full_cfg = _load_full_config()
    full_cfg.pop(f"{_SIGMA_PFX}{tk}", None)
    try:
        _CONFIG.write_text(json.dumps(full_cfg, ensure_ascii=False, indent=2), encoding="utf-8")
    except:
        pass
...
_is_stdev_title = (_strategy_title == "📐 표준편차매매")
_is_sigma_title = (_strategy_title == "📐 Sigma매매")
if _is_stdev_title:
    st.title("📐 표준편차매매")
elif _is_sigma_title:
...
with st.sidebar:
    ...
    _is_sigma = (st.session_state.get("strategy_radio", "📐 표준편차매매") == "📐 Sigma매매")
    ...
    if _is_sigma:
        ...
        # Sigma모드에서 사용하지 않는 변수 더미값
        a_buy = a_sell = 0.0; n_days = 2
        sell_ratio = 100.0; divisions = 20
        sd_sigma_period = 2; sd_k_buy = 0.65; sd_k_sell = 0.45; sd_renewal = 5
        start_date = bt_start_date; end_date = bt_end_date; initial_capital = bt_initial_capital
        ticker = bt_ticker
...
def _calc_sigma_params(closes: np.ndarray, end_idx: int, period_days: int):
    """전날까지의 period_days 기간 μ, σ, rolling_max 계산."""
...
""")
    ...
    st.subheader("🔵 매도 전략")
    st.markdown("""
Sigma 매매법은 매도 트리거를 여러 방식으로 설정할 수 있습니다:

| 매도 모드 | 설명 |
|---|---|
| 🎯 트레일링 스탑 | 일정 수익 달성 후 최고점 대비 N% 하락 시 전량 매도 |
| 💹 평단가 기준 익절 | 평균 단가 대비 +N% 도달 시 부분/전량 매도 |
| 📊 1년 고점 기준 % | 1년 최고가 대비 일정 % 회복 시 매도 |
| 💰 직접 가격 입력 | 목표 가격과 매도 비율을 직접 입력 |

**주의: 같은 날 매수+매도 불가** — LOC 특성상 당일 매수한 주식은 당일 매도할 수 없습니다.
""")

    st.markdown("---")
    st.subheader("⚠️ 주의사항")
    st.warning("""
- **백테스트 한계**: 과거 성과가 미래를 보장하지 않습니다.
...
if _is_sigma:
    with tab5:
        ...
        if _IS_CLOUD and not st.session_state.get("logged_in"):
            ...
            with st.container(border=True):
                st.markdown("#### 🗂️ 구글 스프레드시트 연동")
                st.caption("매매 기록을 구글 스프레드시트에 자동으로 저장합니다.")
                ...
                st.write("")
                _btn_gs1_sg, _btn_gs2_sg, _ = st.columns([1, 1, 2])
                with _btn_gs1_sg:
            ...
            with st.expander("🔧 관리자 도구 — 비밀번호 해시 생성 (users 시트 등록용)"):
                ...
                if st.button("🔑 해시 생성", key="sigma_gen_hash"):
                    if _admin_pw_sg:
                        st.code(_hash_password(_admin_pw_sg), language=None)
                        st.caption("👆 위 해시를 복사해서 users 시트의 password_hash 컬럼에 붙여넣으세요.")
                    else:
                        st.warning("비밀번호를 입력해주세요.")
